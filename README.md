...
![greenroom: The Green Room and The Stage](assets/greenroom-preview.png)
...
The parent folder has no `.git/` of its own. It is an organizational container, so one `cd ~/src/<project>/` puts both halves of the project in front of you. greenroom lets any coding agent reach both repos from a single session. When it detects a VS Code-family editor, it also writes a VS Code workspace. See ["One entry point, any editor"](#one-entry-point-any-editor).

## Install

**Requirements:** macOS or Linux. Windows is supported through WSL2, which presents a Linux environment. You need Python 3 and `git` on your `PATH`. The standalone skill install also needs Node, for `npx`. The manual install needs `bash`. greenroom uses POSIX paths and `$HOME` semantics. As a result, native Windows (cmd and PowerShell) is not supported. The script refuses to run there.

There are two ways to install. They use different approaches. A manual `git clone` follows them, for work on greenroom itself.
...
- **The Claude Code plugin** keeps greenroom as a read-only bundle that you do not edit. The bundle stays current. The plugin also adds the `/greenroom:*` slash commands. When you want greenroom to work as shipped and to follow greenroom as it changes, use this option.
...
The install copies the whole skill directory: `SKILL.md`, the script, and the templates. greenroom then works with no other configuration. Invoke it as `/greenroom`, or describe what you want.
...
This registers the skill and the `/greenroom:*` slash commands (`/greenroom:new`, `/greenroom:add`, `/greenroom:sync`).
...
`install.sh` makes a symlink to the skill at `~/.claude/skills/greenroom`. It makes symlinks to the slash commands at `~/.claude/commands/`. The skill directory holds its own `scripts/` and `templates/`. As a result, nothing else needs to be configured. The installer is idempotent. It never overwrites a real file that you own. It also cleans up the layout that earlier versions left.

A manual install registers the commands without the plugin namespace. The examples below read `/greenroom:new`, `/greenroom:add`, and `/greenroom:sync`. After a manual install, invoke them as `/new`, `/add`, and `/sync`. The plugin install is the recommended path. It gives you the namespaced form.
...
The most common case is a run from inside a repo that you already cloned. The path is therefore optional. It defaults to the current directory. The command moves the existing repo into a new parent folder as `<name>-public/`. It then scaffolds `<name>-private/` next to it. The git history and the origin remote stay intact. The command works whether or not the parent folder holds other repos.
...
Add `--with-private-fork` to either command. The script then also scaffolds a `<name>-private-fork/` beside the others. It is a private dev checkout, cloned from the local `-public` repo. Its remote is named `upstream`. As a result, `origin` stays free for a private GitHub repo. The full three-repo shape is `<name>-public` (the stage), `<name>-private-fork` (private dev), and `<name>-private` (the green room).

Both commands run the same script, `skills/greenroom/scripts/greenroom.py`. They run it through its `retrofit` and `new` subcommands. If the default names do not fit, pass `--public-name` or `--private-name`.
...
Every repo sits under the wrapper, so the session can read and edit all of them with no extra configuration. Each repo's `AGENTS.md` loads as the agent touches its files. A launch at the wrapper also keeps your session history in one bucket. It does not fragment across `-public`, `-private`, and the rest.

greenroom produces `AGENTS.md` as its orientation standard. More than 25 agents read it natively, among them Codex, Cursor, Aider, GitHub Copilot, Windsurf, Zed, Warp, Google Jules, Devin, and VS Code. Claude Code reads `CLAUDE.md`, so greenroom writes a thin `CLAUDE.md` pointer (`@AGENTS.md`) that imports the same file. greenroom configures Gemini CLI through `.gemini/settings.json`. Every other agent reads `AGENTS.md` natively, with no other configuration.

VS Code uses the same wrapper rule, but only if you use it. When the script detects a VS Code-family editor, it writes a `<project>.code-workspace` file at the parent root. The signals are `code`, `cursor`, `codium`/`vscodium`, or `windsurf` on your `PATH`. They are also an existing `.vscode/` or `*.code-workspace` in the wrapper.

The workspace file scans the wrapper. It lists every repo it finds as a root, with the canonical repo first. Each repo gets its own Source Control panel. New terminals open at the wrapper. The **Claude Code** task launches `claude` there for you. Each project gets a title-bar color derived from its name. As a result, two open projects never look alike.

To force or skip the file regardless of detection, pass `--workspace` or `--no-workspace`. Wrapper identity itself is stored in an editor-neutral `.greenroom` marker at the wrapper root. As a result, the workspace file is never required. A terminal-only setup gets none. If you prefer a bare shell, a one-line alias does the same job:

```
gr() { cd ~/src/"$1" && claude; }   # or: codex, gemini, aider, …
```

The script also writes a git-excluded `.claude/settings.local.json` into each repo. The file grants that repo's siblings (`../<name>`). This is a safety net for a stray launch *inside* one repo. Even then, the other repos stay reachable. Those private paths never reach the public repo.

**Optional boundary.** Set `GREENROOM_ROOT` to the directory that holds your projects, for example `export GREENROOM_ROOT="$HOME/GitHub"`. greenroom then operates only at or below that directory. It refuses to scaffold at or above it. It is a safety boundary, not a target. greenroom works without it. greenroom also never treats `$HOME`, the filesystem root, or a standard system directory as a wrapper. `~/Documents` and `~/Desktop` are two such directories. This holds regardless of any signal they carry.

## Recovering docs already in public history

If design docs or notes already landed in the public repo, `collect` pulls them back into the private repo. Run it from inside the public repo. The script always sits inside the skill directory. The path below is where a manual install, or `npx skills add -g`, puts it. On a plugin install, the script is at `~/.claude/plugins/` instead. In that case, use the `/greenroom:*` slash commands, or substitute the plugin path:
...
It reads each file from git history and writes a copy into the private repo. It never rewrites public history. Read the plan first. Then run the command again with `--apply`. Commit the result when you are ready.

## Tests

`tests/smoke.sh` makes throwaway repos in a temp dir. It exercises the script's reliability-critical paths:
...
The installer does claim a *dangling* link at the skill's own path. It claims that link only when the dead target still has our `skills/<name>` shape. If the target is an unmounted volume, or anything else, the installer leaves it alone and reports it. Run the tests directly:
...
`skills/greenroom/SKILL.md` holds the agent-facing instructions. The detail is in `skills/greenroom/references/`. The script and the templates are in that same directory. That directory is the entire installable payload. The slash-command definitions in `commands/` are thin triggers that hold no logic. The design notes on why the layout is shaped this way are in [`docs/design.md`](docs/design.md).
