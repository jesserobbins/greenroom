...
# --- 46. resolve_link must not emit a doubled slash for a target directly under /.
...
# --- 47. a real scripts/ DIRECTORY at the shim path is not a link we made. Nothing
...
# --- 48. a 0.1.8 user has BOTH artifacts: the shim dir and the greenroom-setup
...
# --- 49. OS noise must not fail the install. A Finder-created .DS_Store in the old
...
# --- 50. an EMPTY ~/.claude/skills/greenroom (a partially cleaned or interrupted
...
# --- 51. a shim holding ONLY OS noise -- the links already cleaned up, a .DS_Store
...
# --- 52. a COPIED greenroom-setup payload (from `npx skills add ...@greenroom-setup`)
...
# --- 53. a greenroom-setup link REPOINTED by hand at the renamed skills/greenroom
...
# --- 54. the ORIGINAL collision: pre-0.1.8 the skill was named `setup`, so
...
# --- 55. the ancient ROOT symlink is ours whether it points at this clone or
...
# --- 56. migration 2 also drops a DANGLING greenroom-setup link. The old clone was
...
# --- 57. migration 2 also drops a greenroom-setup link into a DIFFERENT checkout
...
# --- 58. ...including the 0.1.4-0.1.7 shape, where ~/.claude/skills/greenroom-setup
...
# --- 59. a checkout with NO skills at all is a partial or corrupt clone, not a
...
# --- 60. a COPIED greenroom at the skill path is a standalone install (`npx skills
...
# --- 61. ...but a STALE copy is not a working install. A pre-0.2 payload has the
...
# --- 62. the declared name is read from the FRONTMATTER, not from anywhere a
...
# --- 63. after a moved clone the COMMAND links dangle with no provable owner, so
...
# --- 64. withholding NEW command links when the skill did not install is only half
...
# --- 65. ...but a WORKING link at one of those generic names is none of our
...
# --- 66. a payload with scripts/ but no templates/ is not a working install either:
...
# --- 67. ownership checks resolve RELATIVE symlink targets. An install whose links
...
# --- 68. a DANGLING link at the skill path is replaced, not skipped. This is what
...
# --- 69. ...but NOT at a command path. new.md/add.md/sync.md are generic names a
...
# --- 70. re-cloning greenroom somewhere new and re-installing is a normal upgrade
...
# --- 71. the SKILL.md path resolver actually resolves. It is the ONLY thing that
...
# --- 72. new/retrofit write a .greenroom marker; sync adds it to a marker-less wrapper ---
mkdir -p "$T/mark"
...
# --- 73. a stray .greenroom in a forbidden dir does NOT make it a wrapper (walk-up guard) ---
fhm="$T/markforbid"
mkdir -p "$fhm"
...
# --- 74. workspace is skipped when no VS Code signal; --workspace / --no-workspace override ---
...
# --- 75. detection writes the workspace when a .vscode/ dir exists (binary absent) ---
mkdir -p "$T/vscode"
...
# --- 76. the block numbers themselves are unique and sequential. Inserting a test
...
# --- 77. every `## [x.y.z]` changelog heading has its reference-link definition.
